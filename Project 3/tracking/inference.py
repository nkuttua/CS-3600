...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in self.allPositions:
            prob = self.getObservationProb(observation, position, i, self.getJailPosition())
            discrete_distributions[i] = prob * self.beliefs[i]
        discrete_distributions.normalize()
        self.beliefs = discrete_distributions
        # raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        discrete_distributions = DiscreteDistribution()
        for i in self.allPositions:
            pos_dist = self.getPositionDistribution(gameState, i)
            for j, k in pos_dist.items():
                discrete_distributions[j] += self.beliefs[i] * k
        self.beliefs = discrete_distributions
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in legal_positions:
            self.particles.extend([i] * (num_particles // len(legal_positions)))
        if num_particles % len(legal_positions):
            self.particles.extend(random.sample(legal_positions, num_particles % len(legal_positions)))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in self.particles:
            probability = self.getObservationProb(observation, position, i, self.getJailPosition())
            weight_distribution[i] += probability
        if weight_distribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [weight_distribution.sample() for i in range(self.numParticles)]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        everything = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(everything)
        for i in range(self.numParticles):
            self.particles.append(everything[i % len(everything)])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in self.particles:
            ...
            for j in range(self.numGhosts):
                ghost = i[j]
                jail = self.getJailPosition(j)
                probability *= self.getObservationProb(observation[j], position, ghost, jail)
            discrete_distributions[i] += probability
        
        if discrete_distributions.total() == 0:
            self.initializeUniformly(gameState)
        else:
            particlesList = []
            for x in range(self.numParticles):
                sample = discrete_distributions.sample()
                particlesList.append(sample)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                newPosDist = self.getPositionDistribution(gameState, newParticle, i, self.ghostAgent[i])
