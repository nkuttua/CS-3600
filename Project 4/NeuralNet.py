...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        inActs.insert(0, 1)
        weightSum = self.getWeightedSum(inActs)
        inActs.pop(0)

        return self.sigmoid(weightSum)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        return self.sigmoidDeriv(weightSum)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i, j in enumerate(self.weights):
            cur = j + (alpha * inActs[i] * delta)
            weights.append(cur)
            totalModification += abs(j - cur)
            j = cur
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for i in self.layers:
            acts = []
            for j in i:
                acts.append(j.sigmoidActivation(act))
            outputValues.append(acts)
            act = acts
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    for i in range(len(nextLayer)):
                        delta = nextLayer[i].weights[neuronNum + 1] * deltas[0][i]
                        error += delta
                    delta = gPrime * error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for i, j in examplesTest:
        neuralNetOut = nnet.feedForward(i)[-1]
        equal = True
        for k in range(len(neuralNetOut)):
            if round(neuralNetOut[k]) != j[k]:
                equal = False
        if equal:
            testCorrect += 1
        else:
