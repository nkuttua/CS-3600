        ...
        for direction in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not self.walls[next_position[0]][next_position[1]]:
                if next_position in state[1]:
                    ...
                    other_corners = state[1]
                successors.append(((next_position, other_corners), direction, 1))
...
def cornersHeuristic(state, problem):
    ...
    walls = problem.walls  # These are the walls of the maze, as a Grid (game.py) TF grid 30x14
    position, corner_list = state
    if not corner_list:
        return 0
    print(corner_list)
    heuristic_total = 0
    ...
    for corner in corner_list:
        ...
        heuristic_val = min(distance, heuristic_val)

    heuristic_total += heuristic_val

    if len(corner_list) > 1:
        corner_distance = 0
        for c1 in corner_list:
            heuristic_val = float('inf')
            for c2 in corner_list:
                if c1 != c2:
                    distance = util.manhattanDistance(c1, c2)
                    heuristic_val = min(heuristic_val, distance)
            corner_distance += heuristic_val
        heuristic_total += (corner_distance / 2)
        ...
        for f1 in foodGrid:
            ...
            for f2 in foodGrid:
                if f1 != f2:
                    ...
                    heuristic_val = min(heuristic_val, distance)
            food_distance += heuristic_val
        heuristic_total += (food_distance / 2)
